    ...
    def attach_to_controller(self, controller):
        ...
        def on_gui_ready(sender, gui):
            ...
            def Recluster_Local_PCAs():
                ...
                def load_text(filepath, dtype, skiprows=0, delimiter=" "):
                    ...
                    with open(filepath, "r") as f:
                        for _ in range(skiprows):
                            f.readline()
                        x = (
                            pd.read_csv(f, header=None, sep=delimiter)
                            .values.astype(dtype)
                            .squeeze()
                        )
                    return x
                ...
                # ]
                # """
                # cmd=[ "-MinClusters",'2',"-MaxClusters",'12'   ,"-MaxClusters",'12',"-MaxClusters",'12']
                cmd += [
                    "-UseDistributional",
                    "0",
                    "-MinClusters",
            ...
            @controller.supervisor.actions.add(
                shortcut="alt+x", prompt=True, prompt_default=lambda: 14
            )
